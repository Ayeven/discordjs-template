...
module.exports = {
	...
	async run(_args, appcommands, client) {
		try {
			...
			if (remote) {
				void console.log('command deploy have already registered on Discord');
				void client.user.setActivity(`/help ${ client.guilds.cache.size}`, { type: 'PLAYING' });
			}

			else {
				const deploy = appcommands.get('deploy');
				await client.guilds.cache.get(guildId).commands.create(deploy);
				void console.log('Command deploy have been registered to Discord');
				void client.user.setActivity(`/help ${ client.guilds.cache.size}`, { type: 'PLAYING' });
			}
